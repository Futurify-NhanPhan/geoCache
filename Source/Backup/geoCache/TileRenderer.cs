    ...
    def renderTile (self, tile, force = False):
		...
		public virtual void RenderTile(IHttpResponse response, ITile tile, bool force)
		{
			...
			if (image == null)
			{
				...
				{
					Trace.WriteLine(string.Format("Cache miss: {0}, Tile: x: {1}, y: {2}, z: {3}, time: {4}", tile.BBox, tile.X, tile.Y,
					                              tile.Z, DateTime.Now - start));
				}
			}
			else
			{
				Trace.WriteLine(string.Format("Cache hit: {0}, Tile: x: {1}, y: {2}, z: {3}, time: {4}", tile.BBox, tile.X, tile.Y,
				                              tile.Z, DateTime.Now - start));
			}
